"""
Native GTK4 report viewer — shows data as Adwaita widgets (no PDF rendering).
Three report types: idea detail, monthly ideas, task detail.
"""
...
# ── Status badge helpers (reuse from ideas_widget) ────────────────────────────
STATUS_BADGE = {
...
class IdeaReportWindow(Adw.Window):
    ...
    def _build(self, idea: Idea):
        ...
        # ── Notas ─────────────────────────────────────────────────────────
        notas = idea.notas or []
        grp_notas = Adw.PreferencesGroup(
            title=f"Notas ({len(notas)})",
        )
        if notas:
            lb = Gtk.ListBox()
...
class MonthlyReportWindow(Adw.Window):
    def __init__(self, ideas, month: int, year: int, parent=None):
        ...
        self._build(ideas, month_name)

    def _build(self, ideas, month_name: str):
        main_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        self.set_content(main_box)
        ...
        if not ideas:
            ...
            for idea in ideas_sorted:
                ...
                badge = _make_badge(
                    idea.estatus,
                    STATUS_BADGE.get(idea.estatus, "badge-gray"),
                )
        ...
        for estatus in STATUS_LIST:
            ...
            badge = _make_badge(str(n), STATUS_BADGE.get(estatus, "badge-gray"))
    ...
    def _build(self, tarea: Tarea):
        ...
        if historial:
            ...
            for h in reversed(historial):
                ...
                row.add_prefix(
                    _make_badge("●", STATUS_BADGE.get(h.estatus, "badge-gray"))
                )
                lb.append(row)
            grp_h.add(lb)
        else:
